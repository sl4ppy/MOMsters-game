...
export class Beam implements Collidable {
  ...
  constructor(playerX: number, playerY: number, damage: number = 10, pierce: number = 0, range: number = 350, rotationDuration: number = 3) {
  ...
  private async createBeam(): Promise<void> {
    ...
    try {
      ...
      for (let i = 1; i <= 4; i++) {
        ...
        const texture = await import('pixi.js').then(pixi => pixi.Assets.load(import.meta.env.BASE_URL + `sprites/Beam_full_${paddedNum}.png`))
      ...
      console.log(`Created rotating beam with animation frames, range: ${this.range}, collision radius: ${this.collisionRadius}, rotation speed: ${this.rotationDuration} seconds, offset: 16px`)
  ...
  private createFallbackGraphics(): void {
    ...
    console.log(`Created fallback beam graphics, adjusted range: ${this.range} (was ${originalRange}), collision radius: ${this.collisionRadius}`)
