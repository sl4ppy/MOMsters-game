...
export class WeaponSystem {
  ...
  private fireWhip(weapon: WeaponConfig): void {
    ...
    const enemiesInRange = enemies.filter(enemy => {
      const distance = Math.sqrt(
        Math.pow(enemy.position.x - playerPos.x, 2) + Math.pow(enemy.position.y - playerPos.y, 2)
      );
  ...
  private fireHolyWater(weapon: WeaponConfig): void {
    ...
    projectile.onHitTarget = hitTarget => {
      ...
      const enemiesInRange = enemies.filter(enemy => {
        const distance = Math.sqrt(
          Math.pow(enemy.position.x - hitPos.x, 2) + Math.pow(enemy.position.y - hitPos.y, 2)
        );
        return distance <= areaRadius;
      });
  ...
  private fireEyeBeam(weapon: WeaponConfig): void {
    ...
    const beam = new Beam(playerPos.x, playerPos.y, damage, pierce, finalRange, rotationDuration);
  ...
  private checkBeamEnemyCollisions(beam: Beam): void {
    ...
    for (const enemy of enemies) {
      ...
      if (currentTime - lastHitTime < 200) {
        // 200ms cooldown between hits on same enemy
        continue;
      }
  ...
  private isEnemyInBeamPath(beam: Beam, enemy: any): boolean {
    ...
    const isAlongBeamLength =
      localX + enemyRadius >= beamStartX && localX - enemyRadius <= beamEndX;
