...
export class WaveSpawner {
  ...
  private waveConfigs: WaveConfig[] = [
    { "start_time": 0, "end_time": 1, "enemies": ["Blob"], "spawn_rate": 50.0, "max_count": 50, "event": "Normal" },
    { "start_time": 0.5, "end_time": 2, "enemies": ["Goblin", "Plant"], "spawn_rate": 60.0, "max_count": 60, "event": "Normal" },
    { "start_time": 1.5, "end_time": 2.5, "enemies": ["Hobgoblin"], "spawn_rate": 75.0, "max_count": 70, "event": "Normal" },
    { "start_time": 2, "end_time": 3, "enemies": ["Mermaid", "Gargoyle"], "spawn_rate": 90.0, "max_count": 80, "event": "CircleFormation" },
    { "start_time": 2.5, "end_time": 4, "enemies": ["ChompChest", "TreeEnt"], "spawn_rate": 100.0, "max_count": 90, "event": "Normal" },
    { "start_time": 3.5, "end_time": 5, "enemies": ["Reaper", "Palomino"], "spawn_rate": 110.0, "max_count": 100, "event": "Normal" },
    { "start_time": 4.5, "end_time": 5.25, "enemies": ["Green Dragon"], "spawn_rate": 5.0, "max_count": 1, "event": "BossEvent" },
    { "start_time": 4.75, "end_time": 6, "enemies": ["Red Dragon", "Blue Dragon"], "spawn_rate": 125.0, "max_count": 110, "event": "Normal" },
    { "start_time": 5.5, "end_time": 7, "enemies": ["Skeleton", "Mollusk"], "spawn_rate": 135.0, "max_count": 120, "event": "Normal" },
    { "start_time": 6.5, "end_time": 7.5, "enemies": ["Banshee", "Floating Maw"], "spawn_rate": 150.0, "max_count": 130, "event": "SwarmEvent" },
    { "start_time": 7, "end_time": 8.5, "enemies": ["Cacodemon", "Sea Hag"], "spawn_rate": 160.0, "max_count": 140, "event": "Normal" },
    { "start_time": 8, "end_time": 9.5, "enemies": ["Demon", "Centaur"], "spawn_rate": 175.0, "max_count": 150, "event": "Normal" },
    { "start_time": 9, "end_time": 10, "enemies": ["Green Orc", "Golden Orc"], "spawn_rate": 190.0, "max_count": 160, "event": "CircleFormation" },
    { "start_time": 9.5, "end_time": 10.25, "enemies": ["Void"], "spawn_rate": 5.0, "max_count": 1, "event": "BossEvent" },
    { "start_time": 9.75, "end_time": 11, "enemies": ["Golem", "Ice Golem"], "spawn_rate": 200.0, "max_count": 170, "event": "Normal" },
    { "start_time": 10.5, "end_time": 12, "enemies": ["Jawa", "Mud Golem"], "spawn_rate": 210.0, "max_count": 180, "event": "Normal" },
    { "start_time": 11.5, "end_time": 12.5, "enemies": ["Skull", "PlasmaMan"], "spawn_rate": 225.0, "max_count": 190, "event": "SwarmEvent" },
    { "start_time": 12, "end_time": 13.5, "enemies": ["Red Dragon", "Demon", "Void"], "spawn_rate": 240.0, "max_count": 200, "event": "Normal" },
    { "start_time": 13, "end_time": 14.5, "enemies": ["PlasmaMan", "Ice Golem", "Void"], "spawn_rate": 250.0, "max_count": 220, "event": "Normal" },
    { "start_time": 14, "end_time": 15, "enemies": ["Golden Orc", "Void", "PlasmaMan"], "spawn_rate": 275.0, "max_count": 250, "event": "FinalAssault" },
    { "start_time": 14.5, "end_time": 15.5, "enemies": ["Skull King"], "spawn_rate": 5.0, "max_count": 1, "event": "BossEvent" }
  ]
  
  constructor(gameContainer: Container, camera: Camera, collisionManager: CollisionManager, player: PlayerLike, enemySpriteManager: EnemySpriteManager) {
    ...
    console.log(`🎯 Initial wave: ${this.currentWave ? this.currentWave.enemies.join(', ') : 'None'}`)
    console.log(`🎯 Game time: ${this.gameTime.toFixed(3)} minutes`)
    console.log(`🎯 Current wave index: ${this.currentWaveIndex}, Wave: ${this.currentWave ? `${this.currentWave.start_time}-${this.currentWave.end_time}min, spawn_rate: ${this.currentWave.spawn_rate}/s` : 'None'}`)
  }
  ...
  private updateGameTime(deltaTime: number): void {
    ...
    if (this.gameTime < 5.0 && Math.floor(this.gameTime * 20) > Math.floor(previousGameTime * 20)) {
      console.log(`⏰ Game time: ${this.gameTime.toFixed(3)} minutes (deltaTime: ${deltaTime.toFixed(1)}ms) - Real time passed: ${(this.gameTime * 60).toFixed(1)}s`)
      console.log(`⏰ Current wave should be: ${this.getCurrentExpectedWaveIndex()} (${this.getExpectedWaveName()})`)
    }
  ...
  private updateCurrentWave(): void {
    ...
    if (!this.currentWave) {
      ...
      console.log(`⚠️ gameTime: ${this.gameTime}, previousWave: ${previousWave ? `${previousWave.start_time}-${previousWave.end_time}` : 'null'}`)
    }
    ...
    if (previousWaveIndex !== this.currentWaveIndex && this.currentWave) {
      ...
      console.log(`🌊 Wave ${this.currentWaveIndex + 1}: ${this.currentWave.enemies.join(', ')} (${this.currentWave.event}) - spawn_rate: ${this.currentWave.spawn_rate}/s, max: ${this.currentWave.max_count}`)
      console.log(`🌊 Time range: ${this.currentWave.start_time}-${this.currentWave.end_time} minutes, Current time: ${this.gameTime.toFixed(3)} minutes`)
    }
    ...
    if (this.gameTime < 5.0 && this.currentWave) {
      ...
      if (halfMinuteMarker !== Math.floor((this.gameTime - 0.001) * 2)) {
        console.log(`🎯 Wave Status: gameTime=${this.gameTime.toFixed(3)}min, currentWaveIndex=${this.currentWaveIndex}, currentWave=${this.currentWave.enemies.join(',')}`)
      }
    }
  }
  ...
  private updateSwarmEvent(deltaTimeSeconds: number): void {
    ...
    if (this.eventTimer >= 2.0) { // Burst every 2 seconds
      this.eventTimer = 0
  ...
  private updateFinalAssault(deltaTimeSeconds: number): void {
    ...
    this.eventTimer += deltaTimeSeconds
    if (this.eventTimer >= 0.3) { // Spawn every 0.3 seconds
      this.eventTimer = 0
  ...
  private updateSpawning(deltaTime: number): void {
    ...
    if (aliveEnemyCount >= this.currentWave.max_count) {
      ...
      if (aliveEnemyCount % 10 === 0) {
        console.log(`❌ updateSpawning early return: alive enemies=${aliveEnemyCount} >= max_count=${this.currentWave.max_count} (total in array: ${this.enemies.length})`)
      }
      return
    }
    
    // Skip regular spawning during special events that handle their own spawning
    if (this.currentWave.event === FormationType.SWARM || this.currentWave.event === FormationType.FINAL_ASSAULT) {
    ...
    if (this.timeSinceLastSpawn >= spawnInterval) {
      ...
      if (aliveEnemyCount % 10 === 0 || aliveEnemyCount < 10) {
        console.log(`✅ Spawning enemy! Alive enemies: ${aliveEnemyCount}/${this.currentWave.max_count}, spawn_rate: ${this.currentWave.spawn_rate}/s`)
      }
      this.spawnEnemy()
  ...
  private spawnEnemy(): void {
    if (!this.currentWave || this.currentWave.enemies.length === 0) {
      console.log(`⚠️ Cannot spawn enemy: currentWave=${!!this.currentWave}, enemies=${this.currentWave?.enemies.length || 0}`)
      return
    }
    ...
    if (this.enemies.length % 10 === 0) {
      console.log(`✅ Enemy added to world! ${enemyName} #${this.enemies.length} at (${enemy.sprite.x.toFixed(0)}, ${enemy.sprite.y.toFixed(0)})`)
    }
    
    // Log spawning occasionally
    if (this.enemies.length % 20 === 0) {
      console.log(`Spawned ${enemyName} #${this.enemies.length} (Wave ${this.currentWaveIndex + 1})`)
    }
  }
  ...
  private cleanupDeadEnemies(): void {
    ...
    if (removedCount > 0) {
      console.log(`🧹 Cleaned up ${removedCount} dead enemies. Total alive: ${aliveEnemies.length}`)
    }
  ...
  getStats(): { 
    ...
    const phaseProgress = this.currentWave ? 
      (this.gameTime - this.currentWave.start_time) / (this.currentWave.end_time - this.currentWave.start_time) : 0
  ...
  getWaveInfo(): {
    ...
    const timeRemaining = this.currentWave ? 
      Math.max(0, this.currentWave.end_time - this.gameTime) : 0
